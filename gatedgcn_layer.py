"""
GatedGCN layer — copied from graphgps/layer/gatedgcn_layer.py

Only change: replaced `register.act_dict[act]` with a local dict
to remove GraphGym dependency. Everything else is IDENTICAL.

Original: https://github.com/rampasek/GraphGPS
Paper: Residual Gated Graph ConvNets https://arxiv.org/pdf/1711.07553.pdf
"""
...
# Local replacement for graphgym's register.act_dict
ACT_DICT = {
    'relu': nn.ReLU,
    'gelu': nn.GELU,
    'elu': nn.ELU,
...
class GatedGCNLayer(pyg_nn.conv.MessagePassing):
    ...
    def __init__(self, in_dim, out_dim, dropout, residual, act='relu',
        ...
        self.activation = ACT_DICT[act]                     # <-- only change
